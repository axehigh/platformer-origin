...
`dungeon_tiles.tsx` (collision), `items.tsx`, `enemy.tsx`. It tracks the tilesets instead of
hard-coding gid numbers, so if a tileset changes, regeneration picks up the change
automatically.
...
(run from `C:\skuld\dev_olona\libgdx\platformer-origin\assets\maps\level1`)
...
- `--out PATH` — output `.tmx`; the `tileset source=` is written relative to this path. With
  the invocation above the sources resolve to `level1/dungeon_tiles.tsx` +
  `level1/items.tsx` + `level1/enemy.tsx`.
