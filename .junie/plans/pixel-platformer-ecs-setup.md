...
- Touching a dagger pickup object increments `items` (capped at `maxAmmo`) and removes the pickup entity; the HUD item-tracker icon changes to a dagger icon to reflect the new ammo semantics.
...
- **Reuse `items`/`maxAmmo` as dagger ammo:** Rather than introducing a new field, `PlayerComponent.items` becomes the dagger/shoot-ammo count and `maxAmmo` its cap, minimizing churn to `HudStage`'s existing `x NN/NN` counter; only its icon (`gfx/sword.png` → new `gfx/dagger.png`) changes to match the new meaning.
- **Melee resolved as an instantaneous hitbox, not a projectile:** A new `MeleeAttackSystem` mirrors `PlayerBulletSystem`'s bullet-vs-enemy pattern (look up the enemy family once in `addedToEngine`, then AABB-check) but operates on a short-lived rectangle in front of the player instead of a moving `BulletComponent` entity, since a strike has no travel time.
...
- Implement core systems: `PlayerInputSystem`, `MovementSystem` (AABB grid collision + gravity + double jump + wall climb), `AnimationSystem`, `RenderSystem` (z-sorted `SpriteBatch` draw), `TiledMapRenderSystem`/render integration, `CameraSystem` (flip-screen room logic per `AGENTS.md` §E), `PlayerBulletSystem` (bullet-wall/bullet-enemy resolution).
...
- Add `DaggerPickupComponent` (marker + `amount`) and `PickupSystem`: for each pickup entity, check overlap against the single player entity (looked up in `addedToEngine`, mirroring `PlayerBulletSystem`'s enemy lookup); on overlap, increment `player.items` (capped at `maxAmmo`) and remove the pickup.
...
- Generalize `PickupSystem`'s family to `Family.one(DaggerPickupComponent.class, CoinPickupComponent.class)`; in `processEntity`, branch on which marker is present to increment either `player.items` (capped at `maxAmmo`) or `player.coins` (uncapped), then remove the pickup.
...
- **`PlayerBulletSystem`** (new): bullet-vs-wall and bullet-vs-enemy resolution, entity removal.
...
- Pressing **Y** with `items == 0` spawns no bullet and does not enter cooldown; picking up a dagger object increases `items` (capped at `maxAmmo`) and is reflected immediately in the HUD's dagger icon/counter.
...
- Dagger pickup overlapping the player exactly at `maxAmmo` capacity does not overflow `items` past `maxAmmo`.
...
- Implement `PlayerBulletSystem` handling bullet-vs-collision-layer impact (remove bullet) and bullet-vs-enemy-entity impact (apply damage, remove bullet), plus lifetime-based despawn.
...
- Implement `PickupSystem`: looks up the single player entity in `addedToEngine` (mirroring `PlayerBulletSystem`'s enemy lookup), then for each `DaggerPickupComponent` entity checks AABB overlap against the player; on overlap, increments `player.items` capped at `maxAmmo` and removes the pickup entity.
...
- Generalize `PickupSystem`'s family to `Family.one(DaggerPickupComponent.class, CoinPickupComponent.class)`; branch in `processEntity` on which marker is present to increment either `player.items` (capped at `maxAmmo`) or `player.coins` (uncapped), then remove the pickup entity either way.
