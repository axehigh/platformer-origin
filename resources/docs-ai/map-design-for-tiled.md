...
*   `.opencode/skills/tmx-map-generator` — generates a standalone prototype `.tmx` (a linear chain of rooms with enemies/items and walk-through doorways, plus an **optional** hidden secret room — maps are secret-free by default; pass `--secret` to append one to the right, or `--inside-secret` to carve a 6×8-tile chamber inside the last room instead) so you can test a layout without hand-tracing collision CSVs. Rooms **default to 24×10 tiles** (mobile-oriented, dead-zone scroll under the `BAND_ZOOM` camera); pass `--room-width 30 --room-height 17` for whole-screen desktop rooms. **Sections:** `--section NAME[,ROOM[,COL]]` (repeatable) and `--section-pick N` floor-anchored-stamp reusable **Tiled-authored template sections** — rectangles from the `Rooms`/`room`/`sections` object layer of standalone `.tmx` template canvases in `assets/maps/templates/` (see `--list-sections`; gids bridge from the canvas's Tiled numbering into the generator's own via tileset source basenames) — the generator then runs jump-aware design checks (supported ground + reachability within the jump envelope, warnings only), and `validate_map` relaxes its door-decoration checks for section-stamped maps. **Grid layouts:** `--grid-cols C --grid-rows R` tiles `C×R` rooms over the map (width × height = `C×W × R×H`), links horizontal neighbours with doorways and vertical neighbours with one-way **platform shafts** (§4.7), and places `playerStart` in the bottom-left room; grid maps are always secret-free (no-secret is the generator's default). Run `generate_tmx.py` (from `assets/maps/world1`) with `--rooms N --seed S --out level_05.tmx` or `--grid-cols 2 --grid-rows 2 --seed S --out level_01.tmx`; see the skill's SKILL.md for CLI + conventions.
