...
A plain (non-`Component`, non-`Poolable`) helper class embedded as a field inside components that need a countdown — not an Ashley concept, just a reusable value holder. API: `start(float duration)`, `update(float deltaTime)` (clamped at 0, no-op once done), `isActive()`, `isDone()`, `getRemaining()`, `reset()`. Used by `PlayerComponent` (`shootCooldown`, `meleeCooldown`, `meleeAttack`, `hitInvulnerability`), `ChestComponent` (`disappearTimer`), and `EnemyComponent` (`hitStun`); **any new cooldown/countdown/grace-period field should use `Timer` instead of a raw `float`.**

### `EnemyDamageResolver` (`com.axehigh.platformer.ecs.systems.EnemyDamageResolver`)
A package-private static helper (not a `Component`/`System`) shared by `CollisionSystem` (bullet hits) and `MeleeAttackSystem` (melee hits) so both damage sources apply enemy hit-reaction identically. `applyHit(EnemyComponent enemy, MovementComponent movement, float damage, int knockbackDirection)`: if `enemy.hitStun.isActive()`, the hit is fully ignored (returns `false`, no damage/knockback); otherwise subtracts `damage` from `enemy.health` and, if the enemy survives, sets `movement.velocity` to a knockback pop (`90` u/s horizontal away from the attacker per `knockbackDirection`, `140` u/s vertical hop) and starts `enemy.hitStun` for `0.3s`; returns `true` if the enemy's health reached 0 (caller removes the entity).
...
Systems are added to the `PooledEngine` in this priority order (lower runs first each frame); `MeleeAttackSystem`, `PickupSystem`, `ChestSystem`, and `EnemyContactSystem` share the same priority (`7`) and their relative order among themselves is not meaningful:
...
8. `EnemyContactSystem` — priority `7`
9. `CameraSystem` — priority `8`
10. `AnimationSystem` — priority `10`
11. `TiledMapRenderSystem` — priority `20`
12. `RenderSystem` — priority `30`

This ordering matters: input must run before movement/collision resolve it the same frame; enemy AI must set patrol velocity before `MovementSystem` integrates it; melee/pickup/chest/enemy-contact must run after collision-affecting movement but before the camera snaps and animation/rendering read the resulting state; map rendering must happen before entity rendering so entities draw on top.
