...
A dedicated `DebugRenderSystem` (see `resources/docs-ai/ashley-ecs.md`) draws every live `CollisionComponent` AABB (lime), the static map `collisionRects` (yellow), every `RoomState.rooms` rectangle (cyan, with whichever one is currently active in orange — see §2.N), and the player's current melee strike hitbox (red, from `MeleeAttackSystem.getActiveStrikeBounds()`, visible only while a swing is on a reach-bearing frame — see §2.C.4) via a `ShapeRenderer`, toggled on/off with **SHIFT+D** (see `AGENTS.md` "Debugging"). It is a pure visualization aid — disabled by default, no gameplay effect, and skips all drawing while off.
