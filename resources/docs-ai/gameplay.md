...
All of the countdown fields above (`shootCooldown`, `meleeCooldown`, `meleeAttack`, `hitInvulnerability`, `hurtTimer`) are instances of the reusable `com.axehigh.platformer.util.Timer` helper (`start()`/`update()`/`isActive()`/`isDone()`), not raw `float`s; use it for any new cooldown/countdown/grace-period field instead of hand-rolling a decrement.
...
*   `Timer deathTimer` (Tracks the duration of the death animation; once it elapses, the entity is removed from the engine.)
...
4.  **Jump & Landing Smoke:** A ground jump (one where `movement.grounded == true` at the moment the jump fires) spawns a one-shot smoke puff under the player via `ParticleHelper.spawnSmallSmoke(...)` (`GlobalParticles.SMOKE`, `particles/smoke/ParticleParkSmoke.p`, base scale `5.0`), driven to completion by `ParticleSystem`. The puff spawns at a **random spot under the player** — the spawn X is a uniform random offset across the collision box width (not the fixed center), so consecutive puffs are never in the exact same place. The effect's emitter duration is `500ms` (half of its original `1000ms`), so it spawns particles for ~half a second and the entity is removed once the last particle dies (`ParticleEmitter.isComplete()`), leaving the puff visible ~0.5-0.7s total. The same puff spawns again when the player **lands** from a jump: `MovementSystem` detects the landing frame (`movement.grounded` turns true while `player.jumpCount > 0`, checked just before the reset) and spawns it at a random spot under the feet. Because the check keys on `jumpCount > 0`, a jump must have actually happened — walking off a ledge and falling back down leaves `jumpCount == 0` and spawns nothing. The landing puff's scale grows with fall impact speed (a proxy for fall duration): `5.0` for a short hop, lerping to a max of `10.0` at terminal fall speed (`GameConstants.MaxSpeedY`). Air/double jumps spawn no smoke; the puffs are purely visual.
...
1.  **Open Trigger:** When a live melee strike rectangle (i.e. one on a reach-bearing `ATTACKING` frame — see §2.C.4) overlaps an unopened chest (`ChestComponent.opened == false`), mark it `opened = true`, swap its texture to an "open" variant, and `start()` `disappearTimer` (~0.3s). A chest that's already `opened` has no further reaction to being struck (no repeated coin drops).
...
1.  **Shared resolver:** Both enemy touch (`EnemyContactSystem`) and enemy bullet (`EnemyBulletCollisionSystem`) route player damage through a shared `PlayerDamageResolver.applyHit(playerEntity, player, movement, knockbackDirection)` helper, so both damage sources react identically — the player-side mirror of `EnemyDamageResolver` (§2.I).
...
3.  **Hybrid flip-screen / dead-zone scroll camera (`CameraSystem`):** Every frame, `CameraSystem` finds which room rectangle contains the player's position (updating `RoomState.activeRoomIndex`), then positions the camera **per axis** according to that room's resolved mode:
    ...
    *   **Scroll axis** (room bigger than the viewport on that axis, or mode `SCROLL`): **dead-zone scroll** — the camera holds its current position while the player roams anywhere more than `GameConstants.CAMERA_SCROLL_MARGIN` (`320` px) from a screen edge, and only scrolls once the player crosses that margin (keeping the player at the margin line), clamped so the viewport never shows anything past the room's own edges.
...
5.  **Current level examples:** `level_1_demo.tmx` (1 room `2048x1539`, smaller than the viewport on both axes → static flip framing, whole room always on screen); `level_1_demo_2.tmx` (2 side-by-side rooms each `2048x2176` → flip each, camera snaps instantly at the `x=2048` boundary; each room is narrower than the viewport, so a sliver of the neighboring room stays visible, which is expected); `level_1_demo_3.tmx` (no "Rooms" layer → one whole-map room `4096x2176`, only `256` px wider than the viewport → dead-zone scroll; the camera sits pinned for almost the entire map, so the whole level effectively stays on screen while the player roams).
...
4. **Carrying:** While standing on a moving platform the player is moved by the platform's per-frame delta and then clamped out of any overlapped static map collision (shallowest-axis push-out, re-checked until clean) — so a platform pushing the player into a wall stops the player at the wall face while the platform keeps moving. Landing on a platform also spawns the jump-landing smoke puff, identical to a ground landing (same shared emitter `MovementSystem.spawnLandingSmoke(...)`, random X across the feet, same `5.0`→`10.0` fall-speed scaling), keyed on `jumpCount > 0` at the moment the platform snaps the feet onto its top — so a real jump landing smokes, while riding a platform up or walking onto it does not.
